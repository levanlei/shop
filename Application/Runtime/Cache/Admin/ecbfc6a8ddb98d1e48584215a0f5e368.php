   ...
   <div style="padding: 0 50px;width: 80%">
       ...
       <div class="controls">
           <div  style="width: 60%;float: left">
               <input type="hidden" name="thr_id" class="thr_id" value="<?php echo ($data["type_id"]); ?>">
               <select name="shop_brand">
                   <option value="">请选择商品品牌</option>
                   <?php if(is_array($data["brand"])): foreach($data["brand"] as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>"><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
               </select>
           </div>
       </div>
    ...
    <script>
        $(function () {
            ...
            $("select[name='shop_brand']").change(function () {
                ...
                if(id){
                    $.ajax({
                        url:"<?php echo U('Admin/Index/get_shop_all_type');?>",
                        dataType:"json",
                        async:true,
            ...
            $(document).on("change","select[name='shop_type']",function () {
                ...
                if(id){
                    $.ajax({
                        url:"<?php echo U('Admin/Index/get_shop_all_to_input');?>",
                        dataType:"",
                        async:true,
                        data:{id:id},
                        type: "post",
            ...
            $(".add_ok").click(function () {
                ...
                $.ajax({
                    url:"<?php echo U('Admin/index/add_shop_info');?>",
                    dataType:"json",
                    async:true,
                    ...
                    success:function (data) {
                        if(data){
                            ...
                            $.ajax({
                                url:"<?php echo U('Admin/index/add_shop_all_type');?>",
                                dataType:"json",
                                async:true,
